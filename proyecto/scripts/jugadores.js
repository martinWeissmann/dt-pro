document.addEventListener('DOMContentLoaded', () => {
  ...
  const averageAgeElement = document.getElementById('average-age'); // Elemento donde se mostrará el promedio de edad
  ...
  const loadPlayers = () => {
      ...
      updateAverageAge(players); // Actualizar el promedio de edad al cargar los jugadores
  };
  ...
  const savePlayers = () => {
      ...
      updateAverageAge(players); // Actualizar promedio de edad después de guardar
  };
  ...
  const updateAverageAge = (players) => {
      if (players.length === 0) {
          averageAgeElement.textContent = '0';
          return;
      }
      const totalAge = players.reduce((sum, player) => sum + parseInt(player.age), 0);
